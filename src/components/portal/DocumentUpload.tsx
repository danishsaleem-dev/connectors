 ...
 *
 * [fieldName]/[required]/[label]/[hint] let another form reuse this for its
 * own optional-attachment field (e.g. vendor opportunities' attachmentPath)
 * without a near-duplicate component — defaults match the original
 * "add document" usage exactly, so existing callers are unaffected.
 */
export function DocumentUpload({
  ...
  if (!UPLOAD_ENABLED) {
    return (
      <Field label="Link" className="sm:col-span-2">
        <Input name={fieldName} type="url" required={required} placeholder="https://…" />
      </Field>
    );
  }
  ...
  return (
    <Field label={label} hint={hint} className="sm:col-span-2">
      ...
      />
      {path && <input type="hidden" name={fieldName} value={path} />}
